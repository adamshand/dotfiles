...
if status is-interactive
  ...
  function de -d "Exec a command in a container"
    ...
    set -e argv[1..2]  # remove first and second arguments
  ...
  function p -a process -d "Search for matching processes"
    if test $process
      ps $PSARG | grep -i "[^]]$process"
    else
      ps $PSARG | less
    end
  end
  ...
  else if test "$myhostname" = "beam" -o "$HOST_SHORT" = "ch-test1"
    if set -q debug
      echo "HOST: beam or ch-test1 (miro)"
    end
    functions -e mkroot
  end
